...
public class BookSearchByMixController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int totalCount = BookDAO.getInstance().searchByTitleAndAuthorCount(searchStr);
		if (pNo == null) {
			pagingBean = new PagingBean(totalCount);
		} else {
			pagingBean = new PagingBean(totalCount, Integer.parseInt(pNo));
		}
		ArrayList<BookVO> list = BookDAO.getInstance().searchByTitleAndAuthor(searchStr, pagingBean);
		...
		request.setAttribute("command","bookSearchByMix");
